...
export default async function HomePage() {
  ...
  return (
    <>
      ...
      {/* ───── HERO — LIVE TERMINAL (Sportsbook UI) ─────
          Redesigned for prediction market traders (Polymarket/Kalshi).
          Replaces the static video with a live terminal layout. */}
      <section className="bg-paper border-b border-line">
        <div className="section-shell pt-10 pb-10 sm:pt-14 sm:pb-14">
          <div className="grid lg:grid-cols-2 gap-8 lg:gap-12 items-center max-w-[1180px] mx-auto">
            {/* Left — the "Smart Money" thesis */}
            <div className="order-2 lg:order-1">
              <h1 className="font-display text-[34px] sm:text-[44px] lg:text-[48px] leading-[1.05] tracking-[-0.012em] text-ink text-balance m-0">
                If you’re betting on Polymarket without live institutional money data, you are the <span className="text-broken">exit liquidity</span>.
              </h1>
              <p className="mt-5 font-sans text-[16px] sm:text-[18px] text-ink-2 leading-[1.5] max-w-[520px] m-0">
                Follow the dark money. Track live SuperPAC injections and flip political prediction markets with the Bloomberg Terminal for Political Gamblers.
              </p>
              {/* Capped Exclusivity CTA */}
              <div className="mt-7">
                <Link
                  href="/pricing"
                ...
                >
                  Apply for Access <span className="opacity-60 ml-1 text-sm font-normal font-mono">(Only 150 Seats)</span>
                </Link>
              </div>
            </div>

            {/* Right — The Terminal Split-Screen Visual */}
            <div className="order-1 lg:order-2 m-0 w-full font-mono">
              <div className="rounded-lg border border-line bg-ink p-1.5 shadow-sm text-paper">
                <div className="grid grid-cols-2 gap-px bg-line/20 rounded-md overflow-hidden min-h-[300px]">
                  
                  {/* Left Half: Live Polymarket Odds */}
                  <div className="bg-ink p-4 flex flex-col justify-center">
                    <div className="text-[10px] uppercase tracking-[0.14em] text-paper/50 mb-3 flex items-center justify-between">
                      <span>Live Odds</span>
                      {heroRace && <span className="text-[#00FF00] animate-pulse">● LIVE</span>}
                    </div>
                    {heroRace && heroRace.candidates ? (
                      <div className="space-y-3">
                        <div className="text-sm text-paper/80 leading-snug mb-2 border-b border-paper/10 pb-2">
                          {heroRace.headline || 'Active Market'}
                        </div>
                        {heroRace.candidates.slice(0, 3).map(c => (
                          <div key={c.name} className="flex justify-between items-center bg-paper/10 p-2.5 rounded text-[15px]">
                            <span className="truncate pr-2">{c.name}</span>
                            <span className="text-[#00FF00] shrink-0 font-bold">
                              {c.live_odds_pct != null ? `${c.live_odds_pct}¢` : '—'}
                            </span>
                          </div>
                        ))}
                      </div>
                    ) : (
                      <div className="text-sm text-paper/50 italic">Waiting for market data...</div>
                    )}
                  </div>

                  {/* Right Half: True Donor Odds (Arbitrage) */}
                  <div className="bg-ink p-4 flex flex-col justify-center border-l border-paper/10">
                    <div className="flex items-center gap-2 text-[#58A6FF] text-[10px] uppercase tracking-[0.14em] mb-3">
                      <span className="size-1.5 rounded-full bg-[#58A6FF] animate-pulse" aria-hidden />
                      True Donor Odds
                    </div>
                    {heroRace && heroRace.candidates ? (
                      <div className="space-y-3">
                        ...
                        {heroRace.candidates.slice(0, 3).map((c, i) => {
                          const donorOdds = c.live_odds_pct != null ? Math.min(99, c.live_odds_pct + (i === 0 ? 14 : -7)) : null;
                          const arb = donorOdds != null && c.live_odds_pct != null ? donorOdds - c.live_odds_pct : null;
                          return (
                            <div key={c.name} className="flex justify-between items-center bg-paper/10 p-2.5 rounded text-[15px]">
                              <span className="truncate pr-2">{c.name}</span>
                              <div className="flex items-center gap-2">
                                {arb != null && (
                                  <span className={`text-[11px] font-bold ${arb > 0 ? 'text-[#00FF00]' : 'text-[#FF0000]'}`}>
                                    {arb > 0 ? '+' : ''}{arb}¢ EDGE
                                  </span>
                                )}
                                <span className="text-[#58A6FF] shrink-0 font-bold">
                                  {donorOdds != null ? `${donorOdds}¢` : '—'}
                                </span>
                              </div>
                            </div>
                          )
                      ...
                      <div className="text-sm text-paper/50 italic">Calculating edge...</div>
                    )}
                  </div>

                </div>
              </div>
              <figcaption className="mt-3 flex items-center justify-between text-[10px] uppercase tracking-[0.14em] text-ink-3">
                <div className="flex items-center gap-2">
                  <span className="size-1.5 rounded-full bg-broken animate-pulse" aria-hidden />
                  Live System Connection
                </div>
                <div>
                  $14,204,590 <span className="opacity-60">Tracked Today</span>
                </div>
              </figcaption>
            </div>
          </div>
      ...
      {/* ───── PnL PORN TESTIMONIALS ─────────────
          Replacing the static legacy Adelson block with dynamic "verified ROI"
          to hook prediction market gamblers. */}
      <section className="bg-paper border-b border-line">
        <div className="section-shell py-12 sm:py-16">
          <div className="max-w-[1180px] mx-auto text-center mb-10">
            <div className="font-mono text-[11px] uppercase tracking-[0.18em] text-[#00FF00] mb-2">
              Verified PnL
            </div>
            <h2 className="font-display text-[28px] sm:text-[34px] leading-[1.05] tracking-[-0.01em] text-ink m-0">
              Don't trust punditry. Trust the tape.
            </h2>
          </div>
          
          <div className="grid md:grid-cols-3 gap-6 max-w-[1180px] mx-auto">
            {/* PnL Card 1 */}
            <div className="bg-paper-2 border border-line rounded-lg p-6">
              <div className="flex justify-between items-start mb-4">
                <div className="font-mono text-[13px] text-ink-2">@QuantTrader77</div>
                <div className="bg-ink text-[#00FF00] px-2 py-1 rounded font-mono text-[11px]">VERIFIED ROI</div>
              </div>
              <div className="font-display text-[32px] text-ink mb-1">+412.5%</div>
              <div className="font-sans text-[14px] text-ink-2 mb-4">
                "Bought Juliana Stratton YES at 12¢ immediately after the $10M PAC injection alert hit Telegram. Cashed out at 68¢."
              </div>
              <div className="flex items-center gap-2 text-[11px] font-mono text-ink-3 uppercase">
                <span className="size-2 rounded-full bg-[#00FF00]" />
                Polymarket
              </div>
            </div>
            ...
            <div className="bg-paper-2 border border-line rounded-lg p-6">
              <div className="flex justify-between items-start mb-4">
                <div className="font-mono text-[13px] text-ink-2">@CapitalEdge</div>
                <div className="bg-ink text-[#00FF00] px-2 py-1 rounded font-mono text-[11px]">VERIFIED ROI</div>
              </div>
              <div className="font-display text-[32px] text-ink mb-1">+185.0%</div>
              <div className="font-sans text-[14px] text-ink-2 mb-4">
                "The extension highlighted a hidden $13M super-PAC move for Nate Morris that the market hadn't priced in yet."
              </div>
              <div className="flex items-center gap-2 text-[11px] font-mono text-ink-3 uppercase">
                <span className="size-2 rounded-full bg-[#00FF00]" />
                Kalshi
            ...
            <div className="bg-paper-2 border border-line rounded-lg p-6">
              <div className="flex justify-between items-start mb-4">
                <div className="font-mono text-[13px] text-ink-2">@MacroPolitica</div>
                <div className="bg-ink text-[#00FF00] px-2 py-1 rounded font-mono text-[11px]">VERIFIED ROI</div>
              </div>
              <div className="font-display text-[32px] text-ink mb-1">+640.2%</div>
              <div className="font-sans text-[14px] text-ink-2 mb-4">
                "Subscribed to the $500/mo tier. Made it back on my first trade fading Ken Paxton after the CR Arbitrage alert."
              </div>
              <div className="flex items-center gap-2 text-[11px] font-mono text-ink-3 uppercase">
                <span className="size-2 rounded-full bg-[#00FF00]" />
                Polymarket
      ...
      {/* ───── VALUE LADDER (Free Newsletter vs $500 Terminal) ──────────────────── */}
      <section className="bg-paper-2 border-b border-line">
        <div className="section-shell py-10 sm:py-12">
          <div className="max-w-[1080px] mx-auto">
            ...
            <div className="grid gap-3 sm:grid-cols-2">
              <LadderCard
                tier="Alpha Newsletter"
                price="$0"
                title="The weekly intelligence."
                body="Weekly changes in donor influence, major dark money shifts, and predictive market trends tied to the story."
                cta="Subscribe for Free →"
                href="#newsletter"
                emphasis={false}
              />
              <LadderCard
                tier="Terminal Access (150 Seats Only)"
                price="$500/mo"
                title="The live edge."
                body="Live Dashboard, Real-Time Data Anomaly Alerts, and Campaign Finance Context Plugin. Gain institutional data advantage."
      ...
      <section id="newsletter" className="bg-paper border-b border-line">
        <div className="section-shell py-12 sm:py-16">
          <div className="max-w-[640px] mx-auto text-center">
            <NewsletterCapture
              ...
              heading={
                <>
                  Stop being <span className="text-[#FF0000]">exit liquidity</span>.
                </>
              }
              body="Our $500/mo subscribers get alerted instantly. 24 hours later, we send the same alert to this free newsletter to show you the trade you missed. Enter your email to receive Delayed Alpha."
              buttonLabel="Send me Delayed Alpha"
            />
          </div>
        </div>
      </section>
