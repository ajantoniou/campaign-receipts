    ...
    <div ref={ref}>
      ...
      <div className="flex flex-wrap items-end justify-between gap-4 mb-6">
        <div>
          ...
          <div className="font-display text-[44px] sm:text-[64px] leading-none tabular-nums text-broken tracking-[-0.02em]">
            ${counter.toFixed(1)}B
          </div>
        </div>
        <div className="font-sans text-[13px] text-ink-2 leading-snug max-w-[300px]">
          In 1980 the whole election cost $0.3B. By 2024 it cost{' '}
          <strong className="text-ink">$15.9B</strong> — about 53 times more.
        </div>
      </div>

      {/* Chart */}
      <div className="rounded-lg border border-line bg-paper-2 p-4 sm:p-6">
        <div className="relative flex items-end gap-[3px] sm:gap-2 h-[220px] sm:h-[300px]">
              ...
              >
                ...
                >
                  {isLatest ? (
                    ...
                    // Filed total = solid ink bar.
                    <div className="h-full w-full bg-ink-2" />
                  )}
                </div>
                {/* Year label */}
                <div className="mt-1.5 text-center font-mono text-[8px] sm:text-[9px] text-ink-3 tabular-nums">
                  {bar.year}
                </div>
                {bar.estimate && (
                ...
                {/* Hover value */}
                <div className="pointer-events-none absolute -top-1 left-1/2 -translate-x-1/2 -translate-y-full opacity-0 group-hover:opacity-100 transition-opacity bg-ink text-paper font-mono text-[10px] px-1.5 py-0.5 rounded whitespace-nowrap z-10">
                  {fmtMoney(bar.totalB)}
                </div>
              </div>
            )
          ...
          {/* Pro-Israel-PAC overlay marker — drawn as a thin line at the
              ...
              whole. */}
          <div
            className="pointer-events-none absolute left-0 right-0 border-t border-dashed border-broken/60 transition-opacity duration-700"
            style={{
              bottom: `calc(${(udpB / MAX_B) * 100}% + 22px)`,
              opacity: shown ? 1 : 0,
            }}
          >
            <span className="absolute -top-4 right-0 font-mono text-[8px] uppercase tracking-wide text-broken bg-paper-2 px-1">
        ...
        <div className="mt-5 flex flex-wrap gap-x-4 gap-y-1.5">
          {SECTORS.map((s) => (
            <span key={s.key} className="inline-flex items-center gap-1.5 font-mono text-[9px] sm:text-[10px] uppercase tracking-wide text-ink-2">
              <span className="size-2.5 rounded-sm" style={{ background: s.color }} aria-hidden />
              {s.label}
            </span>
          ))}
          <span className="inline-flex items-center gap-1.5 font-mono text-[9px] sm:text-[10px] uppercase tracking-wide text-ink-2">
            <span className="size-2.5 rounded-sm bg-ink-2" aria-hidden />
            Filed total
          </span>
      ...
      <div className="mt-4 overflow-x-auto">
        <div className="flex gap-2 min-w-max pb-1">
          {BARS.filter((b) => b.bill).map((b) => (
            <div
              key={b.year}
              className="flex items-center gap-2 rounded-md border border-line bg-paper px-3 py-2 shrink-0"
            >
              <span className="font-display text-[15px] tabular-nums text-broken leading-none">
                {b.bill!.tag}
              </span>
