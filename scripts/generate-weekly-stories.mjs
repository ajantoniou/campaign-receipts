...
3a. THE VOTE / SPONSOR EXPOSÉ FRAME (use when story_type is "vote_expose" or "expose"): the DATA gives you action + bill_effect + matched donors. State the three facts plainly: (a) the action — "[member] voted for" (vote_expose) or "[member] sponsored" (expose) the bill; for a bloc (bloc_size>1) say "[bloc_size] lawmakers voted for it" and you MAY name a few from bloc_members; (b) WHAT THE BILL DOES — use bill_effect / bill_mechanism verbatim-in-substance, and EXPLAIN WHO IT HELPS in plain terms (e.g. "expanding who counts as an 'accredited investor' widens the pool of customers private-fund managers can sell to — including ordinary retirement savers"). This "why it serves the industry's interest" is FACTUAL policy analysis of the bill text, NOT a motive claim about the lawmaker. (c) the money — the matched industry donors (bloc_top_donors / matched_donors) and amounts. Never say the money caused the vote; lay the three facts side by side and let the reader judge.
