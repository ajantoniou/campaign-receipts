...
// Gemini — quality-sensitive weekly synthesis, runs once/week.
const JOURNALIST_MODEL = 'gemini-2.5-pro'
...
const ai = NO_LLM || !GEMINI_API_KEY ? null : new GoogleGenAI({ apiKey: GEMINI_API_KEY })
...
async function synthesize(candidates, weekOf, alignmentReceipts = []) {
  ...
  // One Gemini call: synthesize top-3 narratives + viral title/hook each.
  ...
  const resp = await ai.models.generateContent({
    ...
    contents: prompt,
    config: {
      systemInstruction: journalistSystem(),
      responseMimeType: 'application/json'
    }
  })
  const usage = resp.usageMetadata || {}
  const cost = ((usage.promptTokenCount || 0) * 1.25 + (usage.candidatesTokenCount || 0) * 5) / 1_000_000
  logCost('gemini/' + JOURNALIST_MODEL, Number(cost.toFixed(4)), `weekly synthesis ${weekOf}`)
...
function buildHtml(weekOf, stories, receipts, articleBySlug) {
  const storyBlocks = stories
    .map((s) => {
      const slug = articleBySlug[s.title]
      const url = slug ? `${SITE}/articles/${slug}` : SITE
      const img = s.heroUrl
...
async function main() {
  ...
  const html = buildHtml(weekOf, stories, receipts, articleBySlug)
  const textBody = [
  ...
  if (DRY) {
    ...
    console.log(`top_story_slugs: ${stories.map((s) => articleBySlug[s.title]).join(', ')}`)
  ...
  const issueRow = {
    ...
    html,
    text_body: textBody,
    top_story_slugs: stories.map((s) => articleBySlug[s.title]),
    receipts_count: receipts.length,
    status: 'built',
    built_at: new Date().toISOString(),
    updated_at: new Date().toISOString(),
  }
  const { error } = await supabase.from('cr_newsletter_issues').upsert(issueRow, { onConflict: 'week_of' })
  if (error) throw new Error(`issue upsert: ${error.message}`)
  console.log(`\n✓ Issue built + persisted for ${weekOf}. Phase 2 (Friday) will send it.`)
}
