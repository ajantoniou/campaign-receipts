...
export async function getCreditState(userId: string): Promise<CreditState> {
    ...
    .upsert(
      {
        ...
        credits_allotment: MONTHLY_ALLOTMENT,
        credits_used: 0,
      },
  ...
  return {
    periodStart,
    periodEnd,
    allotment: created?.credits_allotment ?? MONTHLY_ALLOTMENT,
    used: created?.credits_used ?? 0,
    remaining: (created?.credits_allotment ?? MONTHLY_ALLOTMENT) - (created?.credits_used ?? 0),
  }
}
