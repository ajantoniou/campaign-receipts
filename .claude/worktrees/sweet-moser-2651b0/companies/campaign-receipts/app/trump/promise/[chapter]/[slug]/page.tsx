...
export default function PromisePage({
  ...
  // reasoning text; the deep receipts live in the book itself).
  const receipt =
    p.verdict_reasoning ??
    `This promise was graded ${verdictLabel} in the SEALED Press book. Full case-study receipts at sealed2016.com.`
  ...
  const apaCitation =
    `Oliver, P. (2026). ${p.promise_text} [Verdict: ${verdictLabel}]. ` +
    `In SEALED: The 2016 Promises—Before the Deals ` +
    `(Chapter ${chapter?.number ?? ''}: ${chapter?.short ?? ''}). ` +
    `SEALED Press. ${permalink}`
      ...
      <article className="section-shell py-12 max-w-3xl">
        ...
        <section className="mt-10">
          ...
          <p className="mt-4 text-[15px] text-ink-700 leading-relaxed">
            For the full case-study receipts on this promise — the primary sources, dates, dollar figures,
            and the contemporaneous reporting — read Chapter {chapter?.number} ({chapter?.short}) in the SEALED
            Press book at{' '}
            <a
              href={`https://sealed2016.com/?utm_source=campaignreceipts&utm_medium=cross-link&utm_content=trump-promise-${p.chapter_slug}-${p.promise_slug}`}
              className="text-authority-600 hover:text-authority-700 underline-offset-4 hover:underline font-medium"
              target="_blank"
              rel="noopener"
            >
              sealed2016.com
        ...
        <section className="mt-8 pt-4 border-t border-parchment-200">
          ...
          <a
            href={p.promise_source_url ?? 'https://sealed2016.com'}
            target="_blank"
            rel="noopener"
          ...
          >
            {p.promise_source_url ?? 'https://sealed2016.com'}
        ...
        {chapter && (
          <section className="mt-10 rounded-lg ring-1 ring-parchment-200 bg-parchment-50 p-6">
            ...
            <p className="text-[14px] text-ink-800 leading-relaxed">
              This promise lives in Chapter {chapter.number} of <em>SEALED</em>
              {' — '}
              <Link
        ...
        {/* SEALED cross-link footer band — quiet civic-publication register */}
        <aside className="mt-12 border-t border-parchment-200 pt-8">
          ...
          <p className="text-[14px] text-ink-700 leading-relaxed max-w-2xl">
            This is one of 145 promises in <em>SEALED</em>. The book pairs the verbatim quote with the
            receipt — and 81 of them link to a primary source URL. The full scorecard: 46 KEPT · 51
            PARTIAL · 40 BROKEN · 8 READER-DECIDES.
          </p>
          <p className="mt-3 text-[14px] text-ink-700 leading-relaxed">
            <a
              href={`https://sealed2016.com/?utm_source=campaignreceipts&utm_medium=cross-link&utm_content=trump-promise-${p.chapter_slug}-${p.promise_slug}-footer`}
              target="_blank"
              rel="noopener"
              className="text-authority-700 underline underline-offset-4 hover:underline font-medium"
            >
              Read more →
            </a>
            <span className="text-ink-500">&nbsp;or&nbsp;</span>
            <a
              href={`https://creativelabs2016.lemonsqueezy.com/checkout/buy/90308e58-3e24-40fa-b9be-269480ef8767?utm_source=campaignreceipts&utm_medium=cross-link-buy&utm_content=trump-promise-${p.chapter_slug}-${p.promise_slug}-footer`}
              target="_blank"
              rel="noopener"
              className="text-authority-700 underline underline-offset-4 hover:underline font-medium"
            >
