...
## Deploy topology (read before ANY site change)

**sealed2016.com deploys from `companies/concise-sealed/` ONLY** — Render
service `sealed-press` (srv-d7rub9pkh4rs73f2dbd0), branch `main`, autodeploy
on commit. `companies/Sealed/` is a duplicate working folder that is NOT
wired to any deploy. Site changes must land in `companies/concise-sealed/`
(keep `app/`, `lib/`, `components/` byte-identical in both folders until the
two are consolidated). Live SKUs (LS store creativelabs2016, verified
2026-06-11): $25 paperback (product 1061045) + $5 "2024 Deleted Promises"
PDF (product 1043612). The $15 PDF SKU is retired — never reintroduce it.
