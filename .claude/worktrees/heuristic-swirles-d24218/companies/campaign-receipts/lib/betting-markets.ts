...
function eventToMarket(ev: GammaEvent, group: BettingGroup): BettingMarket | null {
...
function classify(title: string): BettingGroup {
...
export async function getBettingSnapshot(): Promise<BettingSnapshot> {
  ...
  try {
    ...
    const tagged = await fetchPoliticsTagEvents(20)
