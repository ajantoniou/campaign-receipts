...
export async function runTurn(opts: {
  ...
  isFirstTurn: boolean
  /** Free tier passes false: DB-only dossier, NO web_search (the only
   *  uncapped COGS). Defaults true so all existing (paid) callers are
   *  unchanged. */
  ...
  const enableWebSearch = opts.enableWebSearch !== false
  ...
  // Free tier omits it entirely (DB-only dossier, zero web COGS). The SDK
  // rejects an empty tools array, so we spread it in only when enabled.
  const tools: Anthropic.MessageCreateParams['tools'] | undefined = enableWebSearch
  ...
  // web searches; we re-send the accumulated turn until it finishes. (When
  // web search is off there are no tool calls, so the loop never iterates.)
