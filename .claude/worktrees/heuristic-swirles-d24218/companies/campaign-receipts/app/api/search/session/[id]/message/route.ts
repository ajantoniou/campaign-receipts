...
export async function POST(request: Request, { params }: { params: { id: string } }) {
  ...
  if (!ent.user) {
    return NextResponse.json(
      { error: 'Sign up free to keep digging.', upgrade_url: '/auth/signin' },
      { status: 401 },
    )
  }
  // Free tier (logged-in non-subscribers) may run follow-ups too — same
  // session, no extra credit — but web context stays OFF (paid feature).
  ...
  // Pass the tier allotment so a free user's meter reads /5, not /100.
  const credits = await getCreditState(ent.user.id, isFree ? FREE_ALLOTMENT : MONTHLY_ALLOTMENT)
