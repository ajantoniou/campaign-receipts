...
import { getCreditState, useOneCredit, MONTHLY_ALLOTMENT, FREE_ALLOTMENT } from '@/lib/search-credits'
...
export async function POST(request: Request) {
  ...
  if (!ent.user) {
    // Free tier still requires a (free) account — sign in / sign up.
    return NextResponse.json(
      { error: 'Sign up free to run a search.', upgrade_url: '/auth/signin' },
      { status: 401 },
    )
  }
  // Free tier: logged-in non-subscribers get FREE_ALLOTMENT searches/mo with
  // web context OFF (the only uncapped COGS). Paid (hasSoftware) get 100 + web.
  const isFree = !ent.hasSoftware
  const allotment = isFree ? FREE_ALLOTMENT : MONTHLY_ALLOTMENT
  ...
  const cappedMsg = isFree
    ? "You've used all 5 free searches this month. Subscribers get 100 a month — and web context on every search."
    : "You've used all 100 searches this month."
  const before = await getCreditState(ent.user.id, allotment)
  if (before.remaining <= 0) {
    return NextResponse.json(
      {
        error: cappedMsg,
        capped_free: isFree,
        upgrade_url: isFree ? '/pricing' : undefined,
  ...
  if (!after) {
    return NextResponse.json(
      { error: cappedMsg, capped_free: isFree, resets_at: before.periodEnd },
      { status: 429 },
    )
  }

  // First Haiku turn → running summary. Free tier: web context OFF (DB-only).
  let turn
  try {
    turn = await runTurn({
      bundle,
