...
// Plausible (privacy-first, no cookies) — Cloud. This is the exact per-site
// script Plausible issued for campaignreceipts.com; the id is public (like the
// CF token above), so it's hardcoded so the install always verifies. Loaded in
// <head> per Plausible's snippet (their verifier checks the <head>).
const PLAUSIBLE_SRC = 'https://plausible.io/js/pa-w_m45ycr1vDQxW_ZKXMdn.js'
...
export default function RootLayout({ children }: { children: React.ReactNode }) {
  return (
    <html lang="en" className={`${instrumentSerif.variable} ${GeistSans.variable} ${GeistMono.variable}`}>
      <head>
        {/* Plausible (Cloud) — privacy-first, no cookies. In <head> per
            Plausible's snippet so their verifier detects it. The per-site id
            is public. defer matches their async install; plausible.init runs
            after the loader. */}
        ...
        <header className="sticky top-0 z-40 border-b border-line bg-paper/95 backdrop-blur-sm">
          <div className="section-shell">
            <div className="h-16 sm:h-18 flex items-center justify-between gap-2 sm:gap-4">
              ...
              {/* Engine-first nav (web-ux-director 2026-06-17): the product
                  IS the engine, so the nav names it. 4 items max. Races /
                  Articles / Newsletter / For Journalists moved to the footer
                  (still crawlable, still in the sitemap). */}
              <nav className="hidden lg:flex items-center gap-0.5 min-w-0" aria-label="Primary">
                <Link href="/" className="font-sans text-[13px] text-ink-2 hover:text-ink px-2.5 py-3 -my-1 rounded-md transition-colors border-b border-transparent hover:border-ink">
                  Search
                </Link>
                ...
                <Link href="/pricing" className="font-sans text-[13px] text-ink-2 hover:text-ink px-2.5 py-3 -my-1 rounded-md transition-colors border-b border-transparent hover:border-ink">
                  Pricing
                </Link>
                <AuthNavButton />
              </nav>
          ...
          <div className="section-shell pt-16 pb-9">
            ...
            <div className="grid gap-10 sm:grid-cols-3 pt-12">
              ...
              <FooterColumn label="Investigate">
                <FooterLink href="/who-funds/pro-israel">Who took pro-Israel PAC money</FooterLink>
            ...
            <div className="mt-12 pt-6 border-t border-paper/15 flex flex-col sm:flex-row gap-4 sm:items-baseline justify-between">
              ...
              <div className="font-mono text-[10px] uppercase tracking-[0.14em] text-paper/50 leading-relaxed max-w-md sm:text-right">
                ...
                <span className="block mt-1 normal-case tracking-[0.06em] text-paper/40">
                  Revenue: newsletter + Donor Intelligence subscriptions.{' '}
