...
import { getCreditState, MONTHLY_ALLOTMENT, FREE_ALLOTMENT } from '@/lib/search-credits'
...
export default async function InvestigatePage() {
  ...
  // Free tier: logged-in non-subscribers get FREE_ALLOTMENT searches/mo.
  const isLoggedIn = !!ent.user
  const isFree = isLoggedIn && !ent.hasSoftware
  const recent = await getRecentActivity(16)
  const creditState = ent.user
    ? await getCreditState(ent.user.id, ent.hasSoftware ? MONTHLY_ALLOTMENT : FREE_ALLOTMENT)
    : null
        ...
        <div className="max-w-[760px]">
          ...
          {!isLoggedIn && (
            <p className="mt-2 font-sans text-[14px] text-ink-3 leading-[1.5]">
              Sign up free for 5 searches a month. Press Desk ($45/mo) adds web context and 100 searches.
            </p>
          )}
          {isFree && (
            <p className="mt-2 font-sans text-[14px] text-ink-3 leading-[1.5]">
              You&apos;re on the free plan: 5 searches a month, the established record.
              Press Desk ($45/mo) adds web context and 100 searches.
            </p>
          )}
        </div>
