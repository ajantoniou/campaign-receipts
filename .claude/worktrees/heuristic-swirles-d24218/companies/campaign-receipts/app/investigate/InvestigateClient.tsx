...
export default function InvestigateClient({
  ...
  hasSoftware: boolean
  /** logged in at all (free or paid) — controls search-vs-signup gate */
  isLoggedIn?: boolean
  /** logged-in non-subscriber on the free plan (5/mo, web off) */
  isFree?: boolean
  ...
  // Deep-link from the homepage search hero: ?type=&id= auto-starts a
  // dossier; ?q= just pre-fills the search box. Runs once on mount.
  ...
  useEffect(() => {
    ...
    if (type && id && FILTERS.some((f) => f.key === type)) {
      startSession({ type, id, label: '', sub: '' })
    } else if (qParam) {
      onChange(qParam)
    }
  ...
  // ── Sign-up gate for ANONYMOUS visitors only. Logged-in free users fall
  //    through to the live search UI (5/mo, web context off). ──
  if (!isLoggedIn) {
    return (
      <div className="max-w-[760px] rounded-lg border border-line bg-paper-2 p-6">
        ...
        <p className="mt-2 font-sans text-[14px] text-ink-3 leading-[1.5]">
          Sign up free — 5 searches a month, no card. Press Desk ($45/mo) adds web context and
          100 searches.
        </p>
        <Link
          href="/auth/signin"
  ...
  return (
    <div className="max-w-[760px]">
      {/* Credits meter */}
      {credits && (
        <div className="mb-2 flex items-center justify-between rounded-lg border border-line bg-paper-2 px-4 py-2.5">
          <span className="font-sans text-[14px] text-ink">
            <strong>{credits.remaining}</strong> of {credits.allotment}{' '}
            {isFree ? 'free ' : ''}searches left this month
          </span>
          {credits.resets_at && (
      ...
      {/* Free-plan note: web context + recent filings are the Press Desk upgrade. */}
      {isFree && (
        <div className="mb-4 flex flex-wrap items-center justify-between gap-2 rounded-lg border border-dashed border-line px-4 py-2.5">
          <span className="font-sans text-[13px] text-ink-2">
            Free plan: the established record, no web context.
          </span>
          <Link
            href="/pricing"
            className="font-mono text-[11px] uppercase tracking-[0.12em] text-broken hover:text-ink underline underline-offset-4 decoration-broken/40 hover:decoration-ink"
          >
            Press Desk → recent filings + web
          </Link>
        </div>
      )}
