...
// FindYourReps — state-only v1 of the engagement-panel-priority ZIP
// widget. Per panel R2: "highest-converting cold-traffic hook in
// civic-tech. Every journalist will test it with their own ZIP in the
// first 10 seconds."
//
// v1 scope: state abbreviation input (typed or via 2-3-digit ZIP
// prefix → state map). Returns up to 3 federal politicians from that
// state (senators + governor + whoever's in the directory). House
// reps require ZIP→district census data — deferred to v2 post-launch.

import { useState, useTransition } from 'react'
import Link from 'next/link'

// 2-3-digit ZIP prefix → state. Compact map sufficient for first-digit
// disambiguation. The first 3 digits of a US ZIP code uniquely (or
// near-uniquely) identify a state — full mapping is ~900 ranges; this
// is a small subset of the most populous regions for v1.
const ZIP3_TO_STATE: Record<string, string> = {
  // Northeast
  '019': 'MA', '020': 'MA', '021': 'MA', '022': 'MA', '023': 'MA', '024': 'MA', '025': 'MA', '026': 'MA', '027': 'MA',
  ...
  '630': 'MO', '631': 'MO', '633': 'MO', '634': 'MO', '635': 'MO', '636': 'MO', '637': 'MO', '638': 'MO', '639': 'MO', '640': 'MO', '641': 'MO', '644': 'MO', '645': 'MO', '646': 'MO', '647': 'MO', '648': 'MO', '650': 'MO', '651': 'MO', '652': 'MO', '653': 'MO', '654': 'MO', '655': 'MO', '656': 'MO', '657': 'MO', '658': 'MO',
...
type RepResult = {
  ...
  scorecard_pending: number | null
  // Repositioning lead metric (null when no finance data → fall back to promise stat).
  donorHeadline?: DonorHeadline | null
}

const TONE_CLASS: Record<DonorHeadline['tone'], string> = {
  broken: 'text-broken',
...
export default function FindYourReps() {
  ...
  function resolveState(input: string): string | null {
    ...
    const zip3 = cleaned.replace(/\D/g, '').slice(0, 3)
    if (zip3.length === 3) return ZIP3_TO_STATE[zip3] || null
    return null
  }
  ...
  async function onSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    if (!state) {
      setError('Enter a 5-digit ZIP code or a 2-letter state code (e.g. 10001 or NY)')
      return
    }
    ...
    startTransition(async () => {
      try {
        const resp = await fetch(`/api/reps?state=${state}`)
        if (!resp.ok) throw new Error('lookup failed')
        const data = await resp.json()
        setResults(data.reps || [])
      } catch {
        setError('Could not look up reps right now. Try again or browse the directory.')
      }
    })
  }
  ...
  return (
    <div className="rounded-md border border-line bg-paper p-5 sm:p-6">
      ...
      <h3 className="font-display text-[22px] sm:text-[26px] leading-[1.15] text-ink m-0">
        Type a ZIP or state. See who funds your reps.
      </h3>
      <p className="mt-2 font-sans text-[14px] text-ink-2 leading-relaxed max-w-lg">
        Senators + governor for your state — with the money behind each one:
        corporate vs. grassroots, top funders, total raised.
      </p>

      <form onSubmit={onSubmit} className="mt-5 flex gap-2 flex-wrap">
        <input
          name="query"
          value={query}
          onChange={(e) => setQuery(e.target.value)}
          placeholder="10001 or NY"
          maxLength={5}
          className="flex-1 min-w-[180px] bg-paper-2 border border-line focus:border-ink focus:outline-none focus:ring-2 focus:ring-ink/20 rounded-md px-3.5 py-2.5 font-sans text-[15px] text-ink placeholder:text-ink-3 transition"
          autoComplete="off"
        />
        <button
          type="submit"
          disabled={isPending}
          className="inline-flex items-center justify-center rounded-full bg-ink text-paper hover:bg-ink-2 disabled:opacity-50 font-sans text-[14px] font-medium px-5 py-2.5 transition-colors border border-ink"
        >
      ...
      {resolvedState && results && (
        <div className="mt-6">
          <div className="font-mono text-[11px] uppercase tracking-[0.18em] text-ink-2 mb-3">
            Your reps for {resolvedState} ({results.length})
          </div>
          {results.length === 0 ? (
            <p className="font-sans text-[14px] text-ink-2">
              No politicians for {resolvedState} in our archive yet.{' '}
              <Link href="/directory" className="text-ink underline underline-offset-4 decoration-line hover:decoration-ink">
              ...
              {results.map((r) => (
                <li key={r.slug}>
                  <Link
                    ...
                    className="flex items-center gap-3 rounded-md border border-line bg-paper-2 hover:bg-paper-3 hover:border-ink-3 transition-all p-3 no-underline"
                  >
                    <div className="flex-1 min-w-0">
                      <div className="font-display text-[16px] leading-[1.2] text-ink truncate">
                        {r.name.replace(/\s*\([^)]*\)\s*$/, '')}
                      </div>
                      <div className="mt-0.5 font-mono text-[10px] uppercase tracking-[0.14em] text-ink-2">
                        {r.branch}
                        {r.party ? ` · ${r.party[0]}` : ''}
                      </div>
                    </div>
                    {r.donorHeadline ? (
                      // Repositioning: lead with WHO funds them, not promises kept.
                      <div className="text-right">
                        <div className={
                          'font-display text-[18px] leading-none tabular-nums tracking-[-0.005em] ' +
                          TONE_CLASS[r.donorHeadline.tone]
                        }>
                          {r.donorHeadline.value}
                        </div>
                        <div className="mt-0.5 font-mono text-[9px] uppercase tracking-[0.14em] text-ink-3">
                          {r.donorHeadline.label}
                        </div>
                      </div>
                    ) : r.scorecard_graded_total != null && r.scorecard_graded_total > 0 && r.scorecard_percentage_kept != null ? (
                      <div className="text-right">
                        <div className={
                          'font-display text-[20px] leading-none tabular-nums tracking-[-0.005em] ' +
                          (r.scorecard_percentage_kept >= 60 ? 'text-kept' : r.scorecard_percentage_kept >= 40 ? 'text-partial' : 'text-broken')
                        }>
                          {Math.round(r.scorecard_percentage_kept)}%
                        </div>
                        <div className="mt-0.5 font-mono text-[9px] uppercase tracking-[0.14em] text-ink-3">
                          kept · {r.scorecard_graded_total} graded
                        </div>
                      </div>
                    ) : r.scorecard_pending && r.scorecard_pending > 0 ? (
                      <div className="text-right">
                        <div className="font-display text-[20px] leading-none tabular-nums tracking-[-0.005em] text-pending">
                          {r.scorecard_pending}
                        </div>
                        <div className="mt-0.5 font-mono text-[9px] uppercase tracking-[0.14em] text-ink-3">
                          tracking
                    ...
                    <span className="font-mono text-[14px] text-ink-3" aria-hidden>→</span>
                  </Link>
                </li>
              ))}
            </ul>
