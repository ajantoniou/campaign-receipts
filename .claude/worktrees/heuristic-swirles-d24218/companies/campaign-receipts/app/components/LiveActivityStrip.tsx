...
export default async function LiveActivityStrip() {
  ...
  return (
    ...
    >
      <div className="section-shell py-2 flex items-center">
        {/* "LIVE" badge on the left. design-pass 2026-06-13: the badge is
            now an OPAQUE paper gutter that sits ABOVE the rail (z-10) with
            its own right-edge fade, so a chip mid-scroll can never show
            through or collide with the label. Previously the badge had a
            transparent bg and items slid visibly under it. */}
        <div className="live-activity-badge shrink-0 relative z-10 inline-flex items-center gap-1.5 bg-paper pr-4 font-mono text-[10px] uppercase tracking-[0.22em] text-broken">
          <span className="size-1.5 rounded-full bg-broken animate-pulse" aria-hidden />
          Live
        </div>

        {/* Ticker rail — design-pass 2026-06-13: the inner overflow box
            carries a WIDER left fade so items fully dissolve before they
            reach the badge gutter, not flush against it. */}
        <div className="live-activity-rail flex-1 min-w-0 overflow-hidden -ml-px">
