...
// Strip leftover SEALED-book sign-off text baked into the imported 2016
// verdict_reasoning data ("Full receipts in the book at sealed2016.com.",
// "read the book at sealed2016.com.") so the now-free briefs never pitch
// the retired book. Removed 2026-06-14.
function scrubBookText(s: string | null | undefined): string {
  if (!s) return ''
  // Drop whole sentences that pitch the retired book or link sealed2016.com,
  // plus the trailing "Full receipts in the book…" sign-off and the
  // "by SEALED Press" attribution. Leaves the factual grading prose intact.
  return s
    .replace(/\s*Full receipts in the book.*$/i, '')
    .split(/(?<=\.)\s+/)
    .filter((sent) => !/sealed2016\.com|\bthe book\b|\bread the book\b|by SEALED Press/i.test(sent))
    .join(' ')
    .replace(/\s+by SEALED Press\b/gi, '')
    .replace(/\s{2,}/g, ' ')
    .trim()
}
...
export default async function PoliticianPage({ params }: { params: { slug: string } }) {
  ...
  // The 2016 Trump promise audit (formerly the SEALED book paywall) is now
  // free on-site, same as every other politician. Removed 2026-06-14.
  ...
  return (
    <>
        ...
        <div className="section-shell pt-10 pb-8 sm:pt-14 sm:pb-12">
          ...
          <div className="grid sm:grid-cols-[140px_1fr] lg:grid-cols-[180px_1fr] gap-6 sm:gap-10 items-start">
            ...
            <div className="min-w-0">
              ...
              {scrubBookText(politician.profile_narrative) && (
      ...
      {/* ───── REMAINING PROMISES ──────────────────────────────────
          Every promise renders free as a full brief — for every politician,
          including Trump. (The SEALED book paywall was removed 2026-06-14;
          the 2016 audit is now free on-site.) */}
...
function FeaturedPromise({
  ...
  const featuredReasoning = scrubBookText(promise.verdict_reasoning)
  if (featuredReasoning) {
    headerRows.push({ k: 'Why this grade', v: featuredReasoning, sans: true })
  }
...
function PromiseBrief({ id, promise }: { id: string; promise: PromiseRow }) {
  ...
  const fullCase = scrubBookText(promise.case_study_narrative)
  const brief = fullCase || scrubBookText(promise.verdict_reasoning)
...
function ReviewTierTag({ tier }: { tier: 'full' | 'standard' | 'book-sourced' | 'pending' }) {
  const map: Record<string, { label: string; variant: 'kept' | 'partial' | 'broken' | 'neutral' }> = {
    ...
    'book-sourced': { label: '2016 case-study audit', variant: 'partial' },
...
function ReviewTierLine({ tier }: { tier: 'full' | 'standard' | 'book-sourced' | 'pending' }) {
  ...
  if (tier === 'book-sourced') {
    return <>Sourced from the 2016 case study (145-promise audit, paper-trail citations on every claim).</>
  }
  if (tier === 'pending') {
