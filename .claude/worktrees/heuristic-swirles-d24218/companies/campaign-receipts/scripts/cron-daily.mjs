...
// Saturday-only ─────────────────────────────────────────────
// The "make this week's video" founder email: ranks the week's articles by
// Plausible pageviews + newsletter clicks and tells the founder which one to
// turn into a YouTube video (with a paste-ready $9 description block). The
// daily 13:00-UTC cron fires Saturday morning ET, so this lands at the right
// time without a separate cron. Idempotent via cr_founder_digest_log.
if (isSaturday) {
  results.saturdayDigest = runStep('saturday-most-viewed', ['scripts/saturday-most-viewed.mjs'])
} else {
  console.log(`\n── saturday-most-viewed skipped (not Saturday; today UTC weekday=${new Date().getUTCDay()})`)
}

// Summary ───────────────────────────────────────────────────
const elapsed = Math.round((Date.now() - startedAt.getTime()) / 1000)
console.log(`\n── summary ────────────────────────────────────────────────`)
console.log(`elapsed=${elapsed}s isMonday=${isMonday} isSaturday=${isSaturday}`)
